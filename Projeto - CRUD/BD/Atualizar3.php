  ...
  <!--Menu-->
    ...
    <?php
        ...
        if (!empty($_FILES['imagem']['tmp_name'])) {
          ...
          $sql = "UPDATE loja SET nome = '$nome', preco = '$preco', qnt = '$qnt', imagem = '$imagem' WHERE id = '$id'";
        //Se o arquivo estiver vazio, não coloca a variavel imagem na atualização
        } else {
          $sql = "UPDATE loja SET nome = '$nome', preco = '$preco', qnt = '$qnt' WHERE id = '$id'";
        }

        $result = $conexao->query($sql);

        //Recupera a imagem do banco de dados para exibição
        $sql_select = "SELECT imagem FROM loja WHERE id = '$id'";
        $result_select = $conexao->query($sql_select);
        $row = $result_select->fetch_assoc();
        $imagemExibicao = base64_encode($row['imagem']);
    ?>

    <div class="container d-flex align-items-center justify-content-center flex-column">
        <h1 style="margin: 30px">Dados do Produto Atualizados com Sucesso!!!</h1>
        <div class="card text-center" style="width: 18rem;">
            <img src="data:image/jpeg;base64,<?php echo $imagemExibicao; ?>" class="card-img-top" alt="Imagem do Produto">
            <div class="card-body">
                <h5 class="card-title"><?php echo $nome?></h5>
                <p class="card-text">ID: <?php echo $id?></p>
                <p class="card-text">Preço: R$<?php echo $preco; ?></p>
                <p class="card-text">Quantidade: <?php echo $qnt; ?></p>
            </div>
        </div>
        <br>
        <a href="../ADM/Atualizar.html"><button class="btn btn-danger">Voltar</button></a>
    </div>
